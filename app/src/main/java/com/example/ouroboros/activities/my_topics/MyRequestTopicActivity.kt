...
class MyRequestTopicActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_my_request_topic)
        ...
        this.rv_my_request_topics.layoutManager = LinearLayoutManager(
            this.applicationContext,
            RecyclerView.VERTICAL,
            false
        )

        this.rv_my_request_topics.setHasFixedSize(true)


        this.rv_my_request_topics.adapter = requestCouplingsRVAdapter
        ...
        btEmptyRequest?.setOnClickListener {
            var allIsWaiting : Boolean = true
            for (myCoupling in allMyCouplings){
                if (myCoupling.coupledState == ENABLE){
                    allIsWaiting = false
                    break
                }
            }
            if (allIsWaiting){
                        ...
                        setPositiveButton(
                            ...
                            if (validator.isConnected(context)){
                                ...
                                for (myCoupling in allMyCouplings){
                                    ...
                                    when (topic.role_type){
                                        HELPER -> {
                                            topicsTable.updatePublicationType(myCoupling.idApplicantTopic,
                                                allCouplingTopics[index].publication_type - POST_REQUESTED)
                                        }
                                        APPLICANT -> {
                                            topicsTable.updatePublicationType(myCoupling.idHelperTopic,
                                                allCouplingTopics[index].publication_type - POST_REQUESTED)
                                        }
                                    }
                                    index += 1
                ...
                Toast.makeText(this, getString(R.string.msg_error_deleting_coupling_enable), Toast.LENGTH_SHORT).show()
            }
        }
    }
